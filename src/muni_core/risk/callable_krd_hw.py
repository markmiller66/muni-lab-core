# FILE: src/muni_core/risk/callable_krd_hw.py
#
# PURPOSE:
...
def _apply_triangular_bump(
    ...
    *,
    ...
    """
    Triangular bump centered at key_tenors[key_idx], tapering to 0 at adjacent key tenors.

    If key tenor is interior:
        support = [left_key, right_key]
        weight ramps 0->1 from left_key to center, then 1->0 from center to right_key

    For edge key tenors:
        support = [center, neighbor] on one side only (half-triangle).
    """
    out = dense_df.copy()
    tc = _tenor_col(out)
    rc = _rate_col(out)
    ...
    for i, ti in enumerate(t):
        ...
        if left is None and right is not None:
            # left edge: half-triangle on [k, right]
            if k <= ti <= right:
                w = (right - ti) / (right - k) if right != k else 0.0
        elif right is None and left is not None:
            # right edge: half-triangle on [left, k]
            if left <= ti <= k:
                w = (ti - left) / (k - left) if k != left else 0.0
        else:
            # interior: full triangle on [left, right]
            if left <= ti <= k:
                w = (ti - left) / (k - left) if k != left else 0.0
            elif k < ti <= right:
                w = (right - ti) / (right - k) if right != k else 0.0
...
def compute_callable_krd_hw(
    *,
    ...
    for j, k in enumerate(key_tenors):
        ...
        pu = float(price_callable_bond_hw_from_bond_dense_override(
            bond=bond, asof=asof, dense_df=df_up, a=a, sigma=sigma, oas_bp=base_oas_bp,
            freq_per_year=freq_per_year, step_years=step_years, q=q, time_tolerance=time_tolerance
        ))
        pdn = float(price_callable_bond_hw_from_bond_dense_override(
            bond=bond, asof=asof, dense_df=df_dn, a=a, sigma=sigma, oas_bp=base_oas_bp,
            freq_per_year=freq_per_year, step_years=step_years, q=q, time_tolerance=time_tolerance
        ))
    ...
    if include_parallel_sanity:
        # Avoid circular import by importing inside the block
